...
export const Leaderboard: React.FC<LeaderboardProps> = ({ currentUserId }) => {
  ...
  // Responsive: Only show top 3 in podium on small screens, full list on md+
  // Podium: top 3 for mobile, top 5 for md+
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
  const podiumCount = isMobile ? 3 : 5;
  const podium = leaderboard.slice(0, podiumCount);
  // Podium order logic (center 1st)
  const orderMap = [
    [],
    [0],
    [1,0],
  ...
  return (
    <div className="space-y-6">
      <div className="mt-8" />
      {/* Podium: always visible, but only top 3 on mobile */}
      {leaderboard.length > 0 && (
        <div className="flex justify-center items-end gap-8 mb-12">
          {displayOrder.map((podiumIdx, flexIdx) => {
      ...
      {/* Full Leaderboard List: always visible */}
